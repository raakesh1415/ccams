<x-layout>
    <x-slot name="header">
        <h2>Sukan / Permainan</h2>
    </x-slot>

    <div class="club-details-container">
        <h2>Explore Sukan / Permainan Clubs</h2>
        
        <!-- Add Club Button -->
        <div class="add-club-button">
            <button type="button" class="btn btn-primary" onclick="window.location.href='/club/add'">
                Add Club
            </button>
        </div>
        
        <!-- Include club list or additional information specific to Sukan / Permainan -->
        <p>Here, you can find all clubs categorized under "Sukan / Permainan".</p>
        <!-- List of Sukan / Permainan clubs, if available -->
    </div>
</x-layout>
